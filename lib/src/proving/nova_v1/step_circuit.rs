...
//!
//! ## State Vector (arity = 4)
//!
//! ```text
//! z = [commitments_root, nullifiers_root, historic_root, tx_count]
//! ```
//!
...
//! - `tx_count`         — Running count of processed transactions
//!
...
pub mod nova_step_circuit {
    ...
    use crate::proving::nova_v1::{
        ...
        merkle::{
            verify_imt_non_inclusion_circuit, verify_merkle_inclusion_circuit,
            AllocatedImtNonInclusion, AllocatedMerkleHop, ImtNonInclusionWitness,
            MerklePathHop,
        },
    };
    ...
    ///
    /// State vector: `[commitments_root, nullifiers_root, historic_root, tx_count]`
    pub const ROLLOUP_ARITY: usize = 4;
    ...
    pub const DEFAULT_RANGE_BITS: usize = 253;
    ...
    pub struct RollupStepCircuit<F: PrimeField + PrimeFieldBits> {
        ...
        /// New nullifiers root after this transaction. **Not bound by an
        /// insertion gadget yet** (follow-up to this work item); a malicious
        /// proposer cannot remove existing nullifiers because the next
        /// step's non-inclusion proof for any retained nullifier would
        /// fail, but full chain-of-trust requires the IMT-insertion
        /// gadget.
    ...
    impl<F: PrimeField + PrimeFieldBits> RollupStepCircuit<F> {
        ...
        /// that haven't been updated to supply witnesses yet. The
        /// soundness gadgets will reject these proofs once a non-trivial
    ...
    impl<F: PrimeField + PrimeFieldBits> StepCircuit<F> for RollupStepCircuit<F> {
        ...
        fn synthesize<CS: ConstraintSystem<F>>(
            ...
            // ----------------------------------------------------------------
            // 2. Nullifier IMT non-inclusion (gated on `enabled`).
            ...
            //
            //    NOTE: This proves non-inclusion in the *pre-state* root.
            //    A separate IMT-insertion gadget (follow-up) is needed to
            //    also bind `old_nullifiers_root → new_nullifiers_root`.
            // ----------------------------------------------------------------
    ...
    mod tests {
        ...
        fn step_real_tx_with_valid_witnesses_satisfies() {
            ...
            let circuit = RollupStepCircuit::<F1>::new_real(
                depth,
                new_commitments_root,
                F1::from(999u64), // new_nullifiers_root (not bound yet — TODO insertion gadget)
                F1::from(7u64),   // new_historic_root (passed through)
                commitment,
                commitment_path,
                nullifier_witness,
            );

            let z_in = [F1::ZERO, old_nullifiers_root, F1::ZERO, F1::from(5u64)];
            ...
            assert_eq!(z_out[1], F1::from(999u64));
            assert_eq!(z_out[3], F1::from(6u64), "tx_count must increment");
        }
        ...
        fn step_real_tx_with_tampered_commitment_path_fails() {
            ...
            let z_in = [F1::ZERO, old_nullifiers_root, F1::ZERO, F1::ZERO];
        ...
        fn step_padding_passes_state_through_and_satisfies() {
            ...
            let z_in = [F1::from(11u64), F1::from(22u64), F1::from(33u64), F1::from(44u64)];
            let (satisfied, unsat, z_out) = run_step(&circuit, z_in);
        ...
        fn padding_flag_must_be_a_proper_boolean() {
            ...
            let z = [F1::ZERO, F1::ZERO, F1::ZERO, F1::ZERO];
        ...
        fn ivc_folds_a_real_witness_step() {
            ...
            let z0 = vec![F1::ZERO, old_nullifiers_root, F1::ZERO, F1::ZERO];
            let mut rs =
            ...
            assert_eq!(z_out[3], F1::ONE, "tx_count must advance by 1");
        }
    }
}
